...
from src.point_gamma_report_layout import (
    generate_point_gamma_summary_page as _generate_point_gamma_summary_page,
...
def generate_point_gamma_report(
    report_data,
    output_dir,
    *,
    report_name=None,
    ...
    filename = f"{report_name}.pdf" if report_name else "point_gamma_report.pdf"
    ...
    with PdfPages(pdf_path) as pdf:
        for beam_name, beam_data in report_data.items():
            ...
            if not beam_data.get("layers"):
                logger.warning("No point-gamma layers for beam '%s'. Skipping.", beam_name)
                continue
    ...
    logger.info("Point-gamma report saved to %s", pdf_path)
